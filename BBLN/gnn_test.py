...
def test(model, graph, lf, asax, test_mask, device):
    ...
    print(len(graph.edge_attr_2))
    attr = [0, 0, 0, 0, 0, 0, 0]
    edgeattr = [attr for i in range(len(graph.edge_attr_2))]
    edgeattr = np.array(edgeattr)
    edgeattr = torch.tensor(edgeattr, dtype=torch.float, device=device)
    # print(edgeattr)
    ...
    with torch.no_grad():
        for step in tqdm(range(valid_steps)):
            ...
            # output = model(graph.x, lf, graph.edge_index, valid_edge_id)
            output = model(graph.x, lf, asax, graph.edge_index, edgeattr, valid_edge_id)
...
def main():
    ...
    # ppi_data.get_local_feature(window_size=2)
    ...
    lf = ppi_data.lx
    asax = ppi_data.asax
    nodes_num = int(len(ppi_data.ppi_list) // 2)
    print(nodes_num)
    all_valid_mask = [i for i in range(nodes_num)]
    ...
    model = GAT_Net2(in_len=2000, in_feature=13, gin_in_feature=256, num_layers=1, pool_size=3, cnn_hidden=1).to(device)
    ...
    if args.test_all:
        ...
        test(model, graph, lf, asax, graph.val_mask, device)
    else:
        print("---------------- valid-test1 result --------------------")
        if len(graph.test1_mask) > 0:
            test(model, graph, lf, asax, graph.test1_mask, device)
        print("---------------- valid-test2 result --------------------")
        if len(graph.test2_mask) > 0:
            test(model, graph, lf, asax, graph.test2_mask, device)
        print("---------------- valid-test3 result --------------------")
        if len(graph.test3_mask) > 0:
            test(model, graph, lf, asax, graph.test3_mask, device)
